...
/*   Updated: 2019/03/08 19:16:26 by olrudenk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		show_menu2(t_input *box)
{
	...
	mlx_string_put(box->ptr, box->win, 1010, 260, 13158600,
			"11. Zoom in/out: '+' && '-'");
	mlx_string_put(box->ptr, box->win, 1010, 280, 13158600,
			"12. Increase/decrease value: '/' && '*'");
	mlx_string_put(box->ptr, box->win, 1010, 300, 1361940,
			"Quit: 'esc'");
}
...
void		show_menu(t_input *box)
{
	...
	mlx_string_put(box->ptr, box->win, 1010, 140, 13158600,
			"7. Elevetion projection: 'O'");
	mlx_string_put(box->ptr, box->win, 1010, 160, 13158600,
			"8. Reset to default view: 'D'");
	show_menu2(box);
}
